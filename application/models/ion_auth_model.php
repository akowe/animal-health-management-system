...
class Ion_auth_model extends CI_Model
{
	...
	**/
    ...
    public function hoina_urban()
    {
        $this->trigger_events('hoina_urban');
      $id = $this->session->userdata('email');

        $this->limit(1);
        $this->order_by('id', 'desc');
        $q =  $this->db->get('hoina_urban', array('email'=>$id));
    ...
    {
        $this->trigger_events('farm');
        $id || $id = $this->session->userdata('email');

        $this->limit(1);
        $this->order_by('id', 'desc');
        $q =  $this->db->get('farm', array('email'=>$id));
