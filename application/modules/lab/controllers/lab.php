...
class Lab extends MX_Controller {
    ...
    public function addLab() {
        ...
        /*
          ...
          $category_selected = $this->input->post('lab_test_id');
          $lab_result = $this->input->post('valuee');
          ...
          $lab_name_value = array();
          $lab_name_value = array_combine($category_selected, $lab_result);
          }


          $lab_and_value = array();
          if (!empty($category_selected)) {
          foreach ($lab_name_value as $key => $value) {
          $lab_and_value[] = $key . '*' . $value;
          }
          $category_name = implode(',', $lab_and_value);
          } else {
...
// Validating Price Field
       // $this->form_validation->set_rules('patient', 'Patient', 'trim|min_length[2]|max_length[100]|xss_clean');
        //$this->form_validation->set_rules('chip', 'Chip', 'trim|min_length[2]|max_length[100]|xss_clean');
...
//    }
            ...
            if (empty($id)) {
                ...
                redirect("lab/invoice?id=" . "$inserted_id");
            } else {
                $data = array(
                    // 'category_name' => $category_name,
                    'report' => $report,
    ...
    function getLab() {
        ...
        if (!empty($data['labs'])) {
            $output = array(
                ...
                "recordsTotal" => $this->db->get_where('lab', array('hospital_id' => $this->hospital_id))->num_rows(),
                "recordsFiltered" => $this->db->get_where('lab', array('hospital_id' => $this->hospital_id))->num_rows(),
                "data" => $info
            );
        } else {
            $output = array(
