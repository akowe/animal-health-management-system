    ...
    function getDoctor() {
        ...
        foreach ($data['doctors'] as $doctor) {
            if ($this->ion_auth->in_group(array('admin', 'Accountant', 'Receptionist'))) {
                $options1 = '<a type="button" class="btn btn-info btn-xs btn_width editbutton" title="' . lang('edit') . '" href="doctor/editDoctor?id=' . $doctor->id . '" data-toggle="modal" data-id="' . $doctor->id . '"><i class="fa fa-edit"> </i> ' . lang('edit') . '</a>';
            }
        ...
        if (!empty($data['doctors'])) {
            $output = array(
                ...
                "recordsTotal" => $this->db->get_where('doctor', array('hospital_id' => $this->hospital_id))->num_rows(),
                "recordsFiltered" => $this->db->get_where('doctor', array('hospital_id' => $this->hospital_id))->num_rows(),
                "data" => $info
            );
        } else {
            $output = array(
