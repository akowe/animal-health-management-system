...
class Medicine_model extends CI_model {
    ...
    function getMedicineSebi() {
        $this->db->order_by('id', 'desc');
        $this->db->where('hospital_id', $this->hospital_id);
        $this->db->where('category', 'SEBI');
        $query = $this->db->get('medicine');
        return $query->result();
    }
    function getMedicineHoina() {
        $this->db->order_by('id', 'desc');
        $this->db->where('hospital_id', $this->hospital_id);
        $this->db->where('category', 'HOINA');
        $query = $this->db->get('medicine');
        return $query->result();
    }

    function getMedicineAlldn() {
        $this->db->order_by('id', 'desc');
        $this->db->where('hospital_id', $this->hospital_id);
        $this->db->where('category', 'ALDDN');
        $query = $this->db->get('medicine');
        return $query->result();
    }
