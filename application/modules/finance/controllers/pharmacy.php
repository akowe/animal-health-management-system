...
//        $data['p_n'] = '0';
    ...
    public function addPaymentView() {
        ...
        $data['settings'] = $this->settings_model->getSettings();
        //select medicine from medicine table by category
        $data['mediSebi'] = $this->medicine_model->getMedicineSebi();
        $data['mediHoina'] = $this->medicine_model->getMedicineHoina();
        $data['mediAlldn'] = $this->medicine_model->getMedicineAlldn();
        //select chip from patient table by project
        $data['r'] = $this->pharmacy_model->hoinaChipRural();
        $data['u'] = $this->pharmacy_model->hoinaChipUrban();
        $data['s'] = $this->pharmacy_model->sebiChip();
        $data['a'] = $this->pharmacy_model->alldnChip();
...
//        $chip = $this->input->post('chip');
        ...
        $this->form_validation->set_rules('patient', 'Patient', 'trim|min_length[2]|max_length[100]|required|xss_clean');
        // Validating Price Field
        ...
        if ($this->form_validation->run() == FALSE) {
            echo 'All field with * sign, are required';
    ...
    function editPayment() {
        if ($this->ion_auth->in_group(array('admin', 'Accountant', 'Pharmacist'))) {
            ...
            $data['mediAlldn'] = $this->medicine_model->getMedicineAlldn();
            //select chip from patient table by project
            $data['h'] = $this->pharmacy_model->hoinaChip();
            $data['s'] = $this->pharmacy_model->sebiChip();
            $data['a'] = $this->pharmacy_model->alldnChip();
