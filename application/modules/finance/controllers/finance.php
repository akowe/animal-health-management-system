...
// $amount_by_category = $this->input->post('category_amount');
        ...
        if (!empty($category_selected)) {
            foreach ($category_selected as $key => $value) {
                ...
                $category_type = $this->db->get_where('payment_category', array('category' => $value))->row()->type;
                $cat_and_price[] = $value . '*' . $category_price . '*' . $category_type;
...
//    }
            ...
            if (!empty($d_name)) {
                ...
                $limit1 = $this->doctor_model->getLimit();
                if ($limit1 <= 0) {
                    $this->session->set_flashdata('feedback', 'Doctor Limit Full');
                    redirect('finance/addPaymentView');
                }
...
// Validating Description Field
        $this->form_validation->set_rules('c_price', 'Sales price', 'trim|min_length[1]|max_length[100]|xss_clean');
...
// } 
    ...
    function getPayment() {
        ...
        foreach ($data['payments'] as $payment) {
            ...
            $options2 = '<a class="btn btn-xs invoicebutton" title="' . lang('invoice') . '" style="color: #fff;" href="finance/invoice?id=' . $payment->id . '"><i class="fa fa-file-text"></i> ' . lang('invoice') . '</a>';
        ...
        if (!empty($data['payments'])) {
            $output = array(
                ...
                "recordsTotal" => $this->db->get_where('payment', array('hospital_id' => $this->hospital_id))->num_rows(),
                "recordsFiltered" => $this->db->get_where('payment', array('hospital_id' => $this->hospital_id))->num_rows(),
                "data" => $info
            );
        } else {
            $output = array(
